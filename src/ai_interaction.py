...
async def do_spawn_agents(content: str, session_id: Optional[str] = None, owner: Optional[str] = None) -> Dict:
    ...
    try:
        ...
        if isinstance(data, dict) and "tasks" in data:
            val = data["tasks"]
            tasks = val if isinstance(val, list) else None
        elif isinstance(data, list):
            tasks = data
    except (json.JSONDecodeError, TypeError):
        pass
    ...
    async def _run_branch(i: int, task: str) -> Dict:
        try:
            ...
            out = await asyncio.wait_for(
                _drain_agent_stream(stream),
                timeout=PARALLEL_AGENT_TIMEOUT,
            )
            return {"task": task, "output": out[:6000], "error": None}
        except asyncio.TimeoutError:
            logger.warning(
                f"spawn_agents branch {i} timed out after {PARALLEL_AGENT_TIMEOUT}s"
            )
            return {"task": task, "output": "", "error": f"timed out after {PARALLEL_AGENT_TIMEOUT}s"}
    ...
    branches = await asyncio.gather(*coros)
